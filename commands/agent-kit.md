...
`GODOT=` si el binario no se encuentra. Capture/flow **sin** `--headless`. JSON y harnesses en `res://agent/` del juego (no en el addon, no helpers de playtest en `src/`). Informe: líneas `AGENT_OK` / `AGENT_FAIL` + paths de PNG. Un flow de varios turnos: `try_click` + `repeat` (no un `click` duro a un botón `disabled`).
