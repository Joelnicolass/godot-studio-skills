...
Cargar `godot-layered-architecture` y `godot-composition-first` (shaders/sprites: `assets.md`; animación: `godot-animation`; 3D: MCP Blender solo con OK). Si hay MP local/WiFi: `godot-mp-kit`. Si es online: `godot-mp-kit` y plan de **expansión del addon**. Si no hay MP: no cargues el kit. Si hay tests: `godot-testing`.
