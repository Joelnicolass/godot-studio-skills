...
Load `godot-layered-architecture` and `godot-composition-first` (shaders/sprites: `assets.md`; animation: `godot-animation`). If local/Wi-Fi MP: `godot-mp-kit`. If online: `godot-mp-kit` and a plan to **expand the addon**. If no MP: do not load the kit. If tests: `godot-testing`.
