...
3. DEVELOPMENT STANDARDS:
   ...
   - For a Godot game: composition, `@export`/inspector, Resource `.tres` types, UI in the product language and IDs in English. Cite `godot-layered-architecture` and `godot-composition-first`. Shaders: Godot Shaders and Shadertoy (see composition `assets.md`). 2D sprites: ask; references; `godot-animation`; Aseprite MCP only with OK. If 3D: ask; Blender MCP ([lab/mcp-server](https://www.blender.org/lab/mcp-server/)) only with OK + references; export glTF. Cite `godot-mp-kit` **only** for local/Wi-Fi MP; if online, state that the addon must be expanded (ENet LAN is not enough); if no MP, do not cite the kit. If there are tests: `godot-testing` (GUT for GDScript, GdUnit4 for C#). Architecture Clean **or** standard, whichever was chosen.
