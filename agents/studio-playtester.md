...
3. Si hay AgentKit: `inspect --unique` primero y usá `%UniqueName` reales. No inventes nodos. Escribí el JSON en `res://agent/flows/` y los helpers en `res://agent/harness/` (`call.harness`). **No** agregues `func agent_*` a scripts de producto. Capturá/flow con `cli.sh` (no `/tmp` SceneTree). Pasá `--fail-on-error`.
