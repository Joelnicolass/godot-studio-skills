...
3. If AgentKit is present: `inspect --unique` first and use real `%UniqueName`s. Do not invent nodes. Capture/flow with `cli.sh` (not `/tmp` SceneTree). Pass `--fail-on-error`.
