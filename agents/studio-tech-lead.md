...
2. **Test scene** (`res://debug/<feature>_<slug>.tscn`): which product packed scenes and `.tres` it instances, the initial state, what has **not** happened yet (the criterion’s outcome), and which InputMap action the playtester will press. A debug weapon is another `.tres` of the same script (more damage), not another `.gd`. If the criterion already starts in the scene the player opens, say so and do not invent a debug scene. This is recorded on the RFC or the FEATURES `F<n>`.
