...
9. Playtest no es tuyo. No agregues a `src/` métodos de spawn / forzar estado / contar / pausar “para el agente” (`agent_*` ni el mismo rol con otro nombre). Eso va en `res://agent/harness/` (skill `godot-agent-kit` del módulo [godot-studio-playtest](https://github.com/Joelnicolass/godot-studio-playtest)).
