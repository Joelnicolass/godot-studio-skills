...
3. Capturá la feature de verdad (AgentKit `capture`/`flow` si el addon está; si no, misma receta que playtest). Una captura estática no basta si el cambio es interacción: ejercé el flujo (vacío, error, CTA).
