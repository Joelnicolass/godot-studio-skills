...
- Playtest ≠ GUT. Si hay `addons/agent_kit/`, `inspect --unique` y `cli.sh flow --fail-on-error` (no `godot -s /tmp`). JSON/harness en `res://agent/`, nunca `func agent_*` en `src/`. Cubrir **todos** los criterios de **este** slice, no un muestreo. Consola `ERROR`/`SCRIPT ERROR`/`AGENT_STEP_ERROR` = FAIL (pegá `AGENT_ERRORS`). Turnos: `try_click` + `repeat`. Visual exige `VISUAL.md` o refs; si faltan, parar y pedirlas.
