...
Enable the **AgentKit** plugin. CLI: `addons/agent_kit/cli.sh PROJECT inspect --unique` and `flow --flow=boot_smoke.json --fail-on-error`. JSON/harness live in the game’s `res://agent/`, not in the addon or `src/`. A long flow (auction, turns) uses `try_click` and `repeat` — see `addons/agent_kit/README.md` and skill `godot-agent-kit`. Do not use `godot -s /tmp` for captures.
